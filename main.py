...
train_data = TensorDataset(train_inputs, train_masks, train_labels) #相當於zip()
...
bert_classifier, optimizer, scheduler = initialize_model(train_dataloader, device, epochs = 1)
model = train(bert_classifier, train_dataloader, device, optimizer, scheduler, val_dataloader, epochs = 1, evaluation = True)

# SAVE MODEL
print('Save model')
torch.save(model, './saved_model/save.pt')
